...
```text
[네모 버튼 디자인 수정사항]
Join the Waitlist 버튼 호버 시 애니메이팅, 이거 about 페이지의 follow와 youtube 버튼과 동일한
애니메이팅 적용된 것 맞아? 아니라면 기존 follow, youtube 버튼과 동일하게 적용해줘.

[play 영역 템플릿 수정사항]
각 트랙 전환을 할 경우, 하단 가사-트랙리스트 영역에서 자동으로 트랙리스트 부분으로
스와이프되어버리는 오류가 발생하고 있어. 문제는 그 뿐 아니라, 스와이프되었으면 위에
불렛 인디케이터도 바뀌어야 하는데, 그러지 않고 있어. 트랙이 바뀐다고 하단 영역이
트랙리스트 영역으로 바뀌는 건 오류야.
차라리 트랙리스트 부분으로 넘어갔다가, 곡 전환 및 스크롤 조작이 가해지지 않은 채로
일정 시간이 지나게 되면 페이드아웃-가사 영역으로 페이드인 처리를 해줘. 동시에 불렛
인디케이터도 변경되어야 하고. (페이드된 시간 동안 가사영역으로 빠르게 스와이프시키면
움직임 없이도 그냥 페이드되는 효과를 줄 수 있을 것 같아)

[제품 팝업 수정사항]
팝업 자체는 괜찮은데, 한 팝업을 열었다가 닫고 다른 팝업을 열게 되면 이전 팝업의 내용이
들어있는 채로 로드되었다가 선택한 것으로 바뀌는 현상이 있어. 원인 파악하고 해결해줘.

[deus ex machina 가사검증]
deus ex machina 각 트랙의 가사 검증을 진행하자. bugs.co.kr의 각 트랙에 등록된 가사를
직접 가져와서 등록해줘. https://music.bugs.co.kr/album/20639149 이렇게 각 앨범마다
bugs는 고유한 링크가 있는데, 검색해서 나온 앨범의 공유 버튼에 링크 복사 영역이 있어.
그걸 통해 들어가면 나오는 곡들에 대해 'a.trackinfo' 태그가 달린 곡 별 상세페이지로 가는
버튼이 있어. 이를 통해 들어가게 되면 공식 가사가 있어. 이걸 그대로 가져올 수 있도록
크롤링 시스템을 구축해줘. 다음에 음반 내게 되었을 때 비슷한 방식으로 제작할 수 있게.
play 영역에 특정 앨범/곡들을 넣으면 자동으로 가사 찾아오고, youtube 음원 파일을 들으며
음성을 인식하고 타이밍에 적합하게 가사의 타이밍 배치 작업을 최대한 정확하게 하도록 만들어줘.

[deus ex machina에 사용된 가사검증방식을 시스템화]
현재의 play 영역에 대해/음악 관련 프로젝트에 대해 음악 가사 등록 방식을 확인하고,
실제 가사를 제대로 반영하고 이를 직접 오디오를 듣고 음성을 인식하고 타이밍에 적합하게
자동으로 가사 배치를 이뤄지게 시스템을 갖춰줘. 상세 타이밍은 editor.html로 변경하면
되니까. editor.html 편집기도 도달 url이 변경되어야 하는거지? 이제 url에
artic.live/projects/프로젝트명 이렇게 처리되니까. 이것도 확인해줘.
```
