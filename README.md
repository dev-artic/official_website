...
* **C. 유튜브 플레이리스트 갱신 자동화 (GitHub Actions Workflow)**:
  * 영상의 통상적인 오후 6시 공개 직후인 매일 오후 6시 10분(KST) 크론 트리거 또는 수동 작동을 통해 `.github/workflows/update-playlists.yml`가 실행됩니다.
...
### 4. Quarterly Webzine 컴포넌트 (`templates/components/quarterly/`)
  ...
  * **로컬 live Notion 및 캐싱**: `functions/.env.local` 또는 `functions/.env`에 `NOTION_API_KEY`/`NOTION_TOKEN`이 없는 경우 로컬 `/api/quarterly-contents`는 실패합니다 (캐시가 이미 존재할 때는 캐시를 즉시 서빙함). Notion에서 아티스트명/앨범명 오타를 고친 뒤 로컬에 캐시 유무와 상관없이 강제 갱신하여 확인하려면 `?live=true` 또는 `?refresh=true` 쿼리를 붙여 요청합니다. 응답 헤더 `X-artic-Data-Source: local-file-cache` (로컬 파일 캐시 히트) 또는 `local-notion-live-fallback` (노션에서 증분 갱신 및 캐싱 완료)을 확인합니다.
  ...
  * **캐싱 구조 및 반영 속도**: Notion API의 지연과 호출 수 한계를 극복하기 위해 멀티 레이어 캐시 구조를 적용합니다. 일반 유저는 Firestore 캐시(`quarterly_cache/archive`)를 조회하여 응답 속도가 비약적으로 단축됩니다 (`X-artic-Data-Source: firestore-cache`). 캐시 갱신은 어드민 대시보드 진입/새로고침 시 실시간 노션 앨범 데이터 및 개별 에세이 페이지들의 `last_edited_time` 타임스탬프를 대조하여 **변경된 페이지만 노션에서 부분 갱신(Selective/Incremental Update)**하는 방식으로 안전하고 신속하게 수행됩니다. 로컬 환경도 5분 간 유효한 로컬 파일 캐시를 활용해 증분 갱신이 일어납니다.
  ...
  * **Notion issue template & metadata automation**: `분기별 결산` data source의 기본 DB template은 `ACHA Issue Template`입니다. 새 row 생성 시 기본으로 `Source Format = Template Draft`, `업로드 확정 = false`, 7개 tier heading/table, 각 table의 header row와 1개 빈 입력 row가 들어갑니다. 운영자는 본문 tier table의 `앨범`, `아티스트`, `장르`, `발매일`, `한줄평`, `추천 수록곡`, `에디터 노트`, `Companion Essay`만 채우면 됩니다. `/admin` Quarterly 탭의 `Auto-Fill Drafts` 또는 `Auto-Fill Issue Properties`가 본문 표의 `발매일`을 기준으로 `Year`, `Quarter`, `Issue`, `Issue Title`, `Slug`, `Published At`, `아카이빙일`, `Excerpt`, `SEO Description`, `Companion Essay Count`, `Companion Essay Tier Labels`, 공개 게이트 속성을 자동 보정합니다. `Publish Archive`는 draft 속성 자동 보정 후 Notion을 재조회하고 Firestore `quarterly_cache/archive`를 갱신하여 `/quarterly/`와 `/quarterly/acha/?issue=<slug>`에 반영하는 운영 트리거입니다. 배치 CLI는 `node scripts/auto_fill_quarterly_issue_properties.js --dryRun`로 사전 점검하고, 실제 반영은 `node scripts/auto_fill_quarterly_issue_properties.js` 또는 단일 페이지 `--pageId=<page-id>`로 실행합니다. Notion API만으로 앱의 New 드롭다운 template을 생성/기본 지정하는 흐름은 완결되지 않으므로, UI에서 template을 만든 뒤 `node scripts/create_quarterly_acha_notion_template.js --pageId=<template-page-id> --title="ACHA Issue Template"`로 본문과 기본 속성을 갱신합니다. 별도 복제용 draft page가 필요하면 같은 스크립트를 인자 없이 실행해 공개 필터에서 제외되는 `ACHA Issue Template Draft` row를 생성할 수 있습니다.
  ...
  * **Highlighted track audio enrichment**: ACHA issue 본문 table의 `추천 수록곡`/`Tracks` 컬럼에는 `#트랙번호 곡명 / #트랙번호 곡명` 형식으로 곡명만 입력합니다. 운영자는 `/admin` Quarterly 탭의 album `Audio` 버튼에서 YouTube Data API 기반 album playlist 후보와 곡 단위 video 후보를 조회하고, 검증된 후보를 `Use`로 저장합니다. Resolver는 검색 순위만 따르지 않고 artist Topic/공식 채널, `Provided to YouTube by` 설명, 곡명 exact match를 우선 가중하며 cover/karaoke/reaction/lyrics/full album/playlist 계열 후보를 감점합니다. 후보가 없거나 애매하면 track row의 manual field에 YouTube URL 또는 video ID를 입력해 `Save Manual`로 저장할 수 있습니다. 저장값은 Firestore `quarterly_youtube_track_overrides` 컬렉션에 `artist+album+track number+title` key로 보존되며, public `quarterlyContents` 응답은 이 cache를 적용해 `highlightedTracks[].youtubeId`를 내려줍니다. ACHA 상세 플레이어는 YouTube IFrame API의 최소 viewport 및 visible-player 요구사항을 충족하도록 열린 트랙 영역 안에 embed를 렌더링하고, 재생 오류나 autoplay 차단 상태를 사용자에게 표시합니다. 자동 검색 실패 또는 애매한 후보는 임의 publish하지 않고 admin review 상태로 남깁니다.
