...
- admin 페이지 백단 연동 및 프론트 정보노출 + 메일 템플릿에 사용되는 정보 호출 연결 여부 확인
- 팝업 정보 로드 오류 재점검 및 모바일 / 데스크탑 레이아웃 소폭 개선 (팝업 높이 - 고정? 유동? 모바일은? 버튼 아래 부분 여백? 모서리 둥글게가 더 어울리는지?)
- 메일 발송 잘 되고 있는지 테스트발송해서 확인
- 음악 가사등록 시스템이 어떻게 구축되어있는지 확인
